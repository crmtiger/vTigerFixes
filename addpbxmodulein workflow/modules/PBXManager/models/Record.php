...
class PBXManager_Record_Model extends Vtiger_Record_Model{
    ...
    public function updateCallDetails($details){
        ...
        $query = "SELECT * FROM vtiger_pbxmanager where sourceuuid='$sourceuuid'";
        $result = $db->pquery($query, array());
        $pbxmanagerid = $db->query_result($result, 0, 'pbxmanagerid');
		if($pbxmanagerid){
			$moduleModel = Vtiger_Module_Model::getInstance('PBXManager');
			$recordModel = Vtiger_Record_Model::getInstanceById($pbxmanagerid, 'PBXManager');
			$recordModel->set('callstatus', $details['callstatus']);
