...
class Leads extends CRMEntity {
	...
	*/
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
	 ...
	 //start change by crmtiger 2-8-2019
	function getQueryForDuplicates($module, $tableColumns, $selectedColumns = '', $ignoreEmpty = false, $requiredTables = array(),$viewname) {
	 //end change by crmtiger 2-8-2019	
		$requiredTables = array("vtiger_leadaddress","vtiger_leadscf");
		...
		$generator->initForCustomViewById($viewname);
		$wherequery = explode('WHERE',$generator->getQuery());
		...
		if (isset($selectedColumns) && trim($selectedColumns) != '') {
			...
			$sub_query .= " WHERE crm.deleted=0 GROUP BY $selectedColumns HAVING COUNT(*)>1";
			 ...
			 //start change by crmtiger 2-8-2019
			$sub_query = "SELECT $tableColumnsString $fromClause $whereClause and $wherequery[1] GROUP BY $tableColumnsString HAVING COUNT(*)>1";
			 //end change by crmtiger 2-8-2019
		}
