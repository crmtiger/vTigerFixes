...
class Vtiger_FindDuplicate_Model extends Vtiger_Base_Model {
	...
	function getListViewEntries(Vtiger_Paging_Model $paging) {
        ...
        $limit = " LIMIT $startIndex, ". ($pageLimit+1);
        $query = $focus->getQueryForDuplicates($module, $tableColumns, '', $ignoreEmpty,$requiredTables,$_REQUEST['viewname']);
	...
	public function getRecordCount() {
		if($this->rows) {
            ...
            if(!self::$query){
                ...
                $focus = CRMEntity::getInstance($module);
                $ignoreEmpty = $this->get('ignoreEmpty');
                self::$query = $focus->getQueryForDuplicates($module, $tableColumns, '', $ignoreEmpty,$requiredTables,$_REQUEST['viewname']);
            }
            $query = self::$query;
